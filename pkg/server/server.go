...
func New(cfg *config.Config, logger *slog.Logger) (*Server, error) {
	...
	if cfg.Store.Backend == "postgres" {
		...
		s.agentRegistry = postgres.NewAgentRegistry(pgStore)
		s.skillRegistry = postgres.NewSkillRegistry(pgStore)
		s.workflowRegistry = postgres.NewWorkflowRegistry(pgStore)
		s.ragRegistry = postgres.NewRAGRegistry(pgStore)
		s.hookRegistry = postgres.NewHookRegistry(pgStore)
		s.ragDocumentRegistry = postgres.NewRAGDocumentRegistry(pgStore)
		s.ftaDocumentRegistry = postgres.NewFTADocumentRegistry(pgStore)
		s.codeAnalysisRegistry = postgres.NewCodeAnalysisRegistry(pgStore)
		s.memoryRegistry = postgres.NewMemoryRegistry(pgStore)
