...
自主 IT 运维的关键难点并不只是模型推理能力不足，而是在异构任务、受限权限和动态基础设施状态下，系统如何把请求分配到正确的推理与执行机制。真实生产事件常常同时涉及结构化诊断、运维知识检索、安全工具执行、配置一致性校验以及对不确定性的保守处理。本文提出 **ResolveAgent**：一个统一的 AIOps 平台，通过**一致性感知的层次化效用校准智能选择器**在 `FTA`、`RAG`、`Skill`、`Direct-LLM` 和受预算约束的 `Multi` 五类执行模式之间进行路由。与现有仅做单阶段标签分类或仅做 LLM 工具选择的系统不同，ResolveAgent 将路由形式化为一个同时覆盖**执行基底选择**与**目标/模型实例选择**的双层受约束优化问题，并显式将任务质量、响应延迟、执行成本、操作风险以及控制平面一致性差异纳入统一目标函数。围绕该目标，本文提出六项新的技术机制：1) 基于能力图与权限快照的可行性裁剪；2) 面向注册表漂移的控制平面一致性正则项与反事实稳定性训练目标；3) 以证据价值为触发条件的有界 `Multi` 路由；4) **分层记忆系统**（Working/Episodic/Long-term 三层架构）；5) **REACTIVE/DELIBERATIVE 双模式混合规划器**；6) **多智能体消息总线与弹性降级机制**（CircuitBreaker + FallbackCascade）；7) **反馈驱动自适应路由与渐进降级机制**（Resilient Selector）。系统实现上，ResolveAgent 将 `Go` 控制平面中的统一注册表、动态路由同步器和 `Higress` 模型路由层，与 `Python` 运行时中的意图估计、上下文增强、`FTA` 评估器、技能执行底座（ToolHub）、分层记忆、混合规划器、DecisionAuditLogger 和弹性机制协同组织为单一真相源架构。基于 2,847 条脱敏生产事件、5,000 条运维问答和 500 个工具执行样例的实验表明，`ResolveAgent-Hybrid` 达到 89.3% 路由准确率、83.5% 目标选择准确率和 25.1 分钟事件 `MTTR`，相较人工流程将 `MTTR` 降低 47%，相较仅基于 LLM 的路由器进一步降低 15%。更重要的是，层次化决策、一致性感知正则和反事实稳定性目标将 3 秒注入式注册表延迟下的路线翻转率从 13.5% 降至 5.6%，在校准、鲁棒性、安全治理和真实应用场景中均表现出显著优势。结果表明，顶会级自主运维研究的核心创新不应局限于”更强的模型”，而应转向**受治理约束的层次化决策、结构化证据消费、控制平面一致性驱动以及弹性伸缩的系统设计**。
...
1. 提出一种**一致性感知的层次化效用校准路由框架**，将运维决策建模为联合选择执行基底与目标实例的双层受约束优化问题，并集成六项核心技术机制：基于能力图与权限快照的可行性裁剪、控制平面一致性正则项与反事实稳定性训练目标、证据价值驱动的有界 `Multi` 路由、分层记忆系统（Working/Episodic/Long-term）、REACTIVE/DELIBERATIVE 双模式混合规划器，以及多智能体消息总线与弹性降级机制（DecisionAuditLogger + CircuitBreaker + FallbackCascade）；
以及 7) 反馈驱动自适应路由与渐进降级机制（Resilient Selector），将路由从一次性决策升级为失败感知的自适应循环
...
6. **分层记忆系统。** `WorkingMemory`（进程内滚动窗口，`max_size=20`）、`EpisodicMemoryClient`（Redis Session 存储）和 `LongTermMemoryClient`（Milvus 向量库）构成三层记忆架构，支持跨 Session 知识沉淀与实时上下文查询的平衡；
7. **混合规划器。** `HybridPlanner` 支持 REACTIVE（ReAct 循环）和 DELIBERATIVE（LLM 分解 + Replan）两种模式，运行时根据任务复杂度、时间约束和工具数量自适应选择；
8. **多智能体消息总线。** `AgentMessageBus` 提供订阅—发布、请求—响应和广播三种消息模式，支持编排器—执行器协作和跨域信息汇聚；
9. **ToolHub。** Schema Registry（工具版本管理）、Capability Map（能力矩阵语义搜索）、Discovery Service（MCP 自动发现）和 Security Policy（权限控制 + 审计）构成完整的工具管理体系；
10. **DecisionAuditLogger。** 异步非阻塞的决策审计日志，记录每条路由决策的完整上下文快照，包括 `timestamp`、`decision_type`、`confidence`、`reasoning` 和 `context_snapshot`，通过后台 Worker 批量写入存储；
11. **弹性机制。** `CircuitBreaker`（熔断器）和 `FallbackCascade`（多级降级：MCP Tool → Native Skill → LLM Direct → Cached Response）确保系统在部分能力缺失时平稳退化而非崩溃式失效。
...
传统路由系统采用 one-shot 决策模式：一次路由、一次执行、成功或失败。然而，真实运维场景中，同一问题往往需要多种方法才能解决——先查文档、再试工具、最后深度分析。本文提出 **Resilient Selector**，将路由从一次性决策升级为反馈驱动的自适应循环：

$$
\text{Route} \rightarrow \text{Execute} \rightarrow \text{Failure} \rightarrow \text{Re-enrich} \rightarrow \text{Route}^* \rightarrow \cdots \rightarrow \text{CodeAnalysis}
$$

形式化地，定义路由会话 $\mathcal{S} = (x, a, \{r_1, r_2, \ldots, r_k\})$，其中 $x$ 为原始请求，$a$ 为代理标识，$r_i$ 为第 $i$ 次路由尝试。每次失败后，系统执行增量上下文重丰富：

$$
\phi'(x, a) = \phi(x, a) \oplus \Delta_{\text{fail}}(r_i, e_i)
$$

其中 $\oplus$ 表示上下文合并操作，$\Delta_{\text{fail}}$ 编码失败路由类型 $r_i$ 和错误信息 $e_i$。重丰富后的上下文使路由决策器能够：(1) 排除已失败的路由；(2) 根据失败类型调整偏好；(3) 向更低特异性但更高鲁棒性的路由降级。

降级路径遵循严格优先级：

$$
\text{Skill} \xrightarrow{\text{fail}} \text{RAG} \xrightarrow{\text{fail}} \text{FTA/Workflow} \xrightarrow{\text{fail}} \text{Code Analysis}
$$

该设计与现有的 FallbackCascade（tool-level 降级）和 HybridPlanner 的 Replan 机制（step-level 重规划）形成三层嵌套弹性架构，分别在路由层、工具层和步骤层提供故障恢复能力。为防止无限循环，系统设置硬上限 $k \leq K_{\max} = 3$ 和总超时 $T_{\text{total}} = 30\text{s}$。
...
**ToolHub** 作为统一工具管理层，在技能执行底座之上进一步提供四类能力：

1. **Schema Registry**：工具版本管理 + Schema 验证，确保每次调用使用已知且经过验证的参数规范；
2. **Capability Map**：能力矩阵 + 语义搜索，通过关键词匹配将请求映射到最合适的工具集合；
3. **Discovery Service**：MCP 自动发现 + 本地工具注册，支持动态扩展工具生态而无需硬编码；
4. **Security Policy**：权限控制 + 使用审计，按 public/sensitive/restricted 三级安全级别管理工具访问。

因此，本文讨论的”工具调用”不是开放式地让模型随意执行外部动作，而是在可声明、可约束、可审计的治理框架内触发有限能力。ToolHub 的引入使工具管理从”散列表格”升级为”可治理的元数据系统”，直接支撑控制平面一致性与安全审计需求。

### 5.8 分层记忆系统

真实运维对话中的上下文信息在不同时间尺度上具有不同的生命周期与价值密度。ResolveAgent 采用三层记忆架构，在成本与能力之间取得平衡：

**第 1 层：Working Memory（进程内滚动窗口）**

采用固定大小的双端队列（`max_size=20`）维护最近对话记录，所有读写操作在进程内完成，延迟在毫秒级。当新条目进入且窗口已满时，最旧条目被自动淘汰。该层提供实时上下文感知能力，适用于高频短时交互。

**第 2 层：Episodic Memory（Redis Session 存储）**

Session 级别的记忆压缩存储，键名前缀为 `session:`，每个 Session 最多存储 100 条记录并启用 `gzip` 压缩。该层支持跨进程访问和 Session 恢复，使系统在长时程运维事件中保持上下文连贯性。当 Session 结束时，记忆被压缩为语义摘要供后续查询。

**第 3 层：Long-term Memory（RAG 向量数据库）**

基于重要性阈值的知识沉淀机制。当记忆条目的 `importance > 0.7` 时，自动向 Milvus 向量库写入，使用 `bge-large-zh` 嵌入模型生成向量表示，集合名为 `long_term_memory`。该层实现跨 Session 的知识复用，使系统能够从历史运维事件中学习并形成组织能力增量。

**记忆流动模型**

```
User Message → Working Memory (实时写入，TTL=3600s)
             → Episodic Memory (Session 结束时压缩写入)
             → Long-term Memory (importance > 0.7 时向量化沉淀)
```

三层记忆的协同设计解决了单一记忆方案的核心矛盾：Working Memory 满足实时性但无法持久化；Long-term Memory 满足知识复用但写入成本高；Episodic Memory 作为桥梁，以可控成本实现 Session 间的上下文连贯性。

### 5.9 混合规划器：REACTIVE 与 DELIBERATIVE 双模式

与仅依赖 selector 的被动式路由不同，ResolveAgent 的 Hybrid Planner 为复杂多步骤任务提供主动规划能力。系统支持两种规划模式，运行时根据任务特征自适应选择：

**REACTIVE 模式：快速响应**

直接进入 ReAct 循环，适用于简单单步请求：

```
Thought: 分析问题 → Action: 执行工具 → Observation: 获得结果 → ...
```

该模式开销最小，延迟最低，但缺乏前瞻性规划能力，适用于意图明确、路径简单的运维查询。

**DELIBERATIVE 模式：深思熟虑**

首先由 LLM 将目标分解为有序步骤序列，然后逐条执行并监控状态：

```
1. LLM 分解目标 → 子步骤列表
2. 执行每步骤 → 监控执行状态
3. 失败时触发 Replan（最多 3 次重试）
4. 基于执行反馈动态调整后续步骤
```

该模式在复杂故障诊断、跨系统修复和多阶段部署场景中显著优于 REACTIVE，尤其当任务具有明确的因果依赖链时。

**Replan 机制**

当 DELIBERATIVE 模式中的某个步骤执行失败时，系统触发 Replan 流程：

```
Create Plan → Execute Steps → Step Failed → Replan → Complete
```

Replan 不是简单重试，而是基于失败原因重新生成子步骤序列。这一机制使系统在部分失败时仍能通过替代路径完成任务，而不是直接进入全局失败。

**模式选择策略**

Hybrid Planner 根据以下信号自动选择模式：

- 请求复杂度（步骤数估计 > 2 时倾向 DELIBERATIVE）
- 时间约束（延迟预算 < 5s 时倾向 REACTIVE）
- 可用工具数量（> 3 个候选工具时倾向 DELIBERATIVE 以避免工具滥用）
- 历史成功率（某模式在相似请求上的表现反馈）

### 5.10 多智能体通信：AgentMessageBus

在复杂运维场景中，单一智能体往往不足以覆盖全部能力需求。ResolveAgent 通过 AgentMessageBus 实现多智能体间的订阅—发布与请求—响应通信：

**消息模式**

1. **Pub/Sub（发布订阅）**：智能体可订阅特定频道（channel），当其他智能体发布消息时，订阅者异步接收。该模式适用于状态广播、事件通知和日志分发。
2. **Request/Response（请求响应）**：携带 `correlation_id` 的请求—响应对，用于等待特定结果的同步交互。该模式适用于跨智能体的工具调用和能力查询。
3. **Broadcast（广播）**：将消息发送到所有已订阅频道的智能体，适用于全局告警和系统级事件。
...
class AgentMessage:
    id: str              # 全局唯一消息 ID
    sender: str          # 发送方智能体 ID
    channel: str         # 目标频道
    content: dict        # 消息内容负载
    correlation_id: str  # 请求—响应对关联 ID
    timestamp: float     # 消息时间戳
```

**订阅管理**

智能体通过 `agent_id + channel` 二元组订阅消息，消息总线维护订阅索引并支持模式匹配（如通配符订阅）。当频道不存在时，订阅请求被延迟直到该频道被首次创建，支持动态频道扩展。

**多智能体协作场景**

AgentMessageBus 使以下协作模式成为可能：

- **编排器—执行器协作**：编排器（MegaAgent）负责任务分解和路由，将子任务通过消息总线分发给专用执行器；
- **跨域信息汇聚**：多个专业智能体分别处理日志、指标、配置等不同数据源，通过消息总线汇聚结果；
- **能力发现**：智能体可在运行时通过消息总线查询其他智能体是否具备特定能力，而非依赖静态注册表。

### 5.11 可观测性增强：DecisionAuditLogger 与弹性机制

**DecisionAuditLogger**

路由决策的可解释性和可审计性是生产系统的核心要求。ResolveAgent 的 DecisionAuditLogger 采用异步非阻塞设计，通过内部队列将每条决策记录写入持久化存储：
...
class AuditRecord:
    ...
    reasoning: str           # 决策理由（自然语言）
    context_snapshot: dict   # 快照：skills_count, code_issues 等
    latency_ms: float        # 决策耗时
```

关键设计点：

- **异步队列**：`AuditRecord` 首先写入内存队列，由后台 Worker 异步批量写入存储，不阻塞主决策流程；
- **完整上下文**：每次决策快照包含当时的技能数量、代码问题标记、语言检测结果等元数据，支持事后重放与根因分析；
- **持久化选项**：支持写入 Go Platform Store（用于生产环境）或本地日志（用于开发调试）。

**CircuitBreaker（熔断器）**

保护下游服务免受级联失败影响。熔断器状态机：

```
CLOSED（正常）→ OPEN（熔断）→ HALF_OPEN（探测）→ CLOSED（恢复）
```

- 当连续失败数达到阈值（`failure_threshold=5`）时，熔断器进入 OPEN 状态；
- OPEN 状态持续 `reset_timeout=30s` 后进入 HALF_OPEN 状态；
- HALF_OPEN 状态下允许一次探测请求，成功则恢复 CLOSED，失败则重新计时。

**FallbackCascade（多级降级）**

当主执行路径失败时，系统按预设顺序尝试替代策略：

```
MCP Tool → Native Skill → LLM Direct → Cached Response → FallbackResult
```

每级降级携带标签和错误信息，若所有策略均失败则返回带有完整错误链的 `FallbackResult(success=False)`。这一设计确保系统在部分能力缺失时仍能提供"最不差"的响应，而不是直接崩溃。

**可观测性闭环**

DecisionAuditLogger、CircuitBreaker 和 FallbackCascade 共同构成完整的可观测性体系：

- AuditLogger 记录"系统做了什么决策，为什么"；
- CircuitBreaker 记录"下游服务何时降级，为何降级"；
- FallbackCascade 记录"降级过程中尝试了哪些替代路径"。

这些日志通过统一观测面输出，与第 5.8 节描述的反馈闭环共同支撑运行时策略自适应更新。
...
本文提出 ResolveAgent：一个将运维路由建模为**一致性感知层次化效用校准决策问题**的统一 AIOps 平台。该框架通过结合层次化执行基底—目标路由、反事实稳定性训练、AI 增强型故障树、安全技能执行和单一真相源控制平面，同时提升了路由质量、事件解决效率、置信度校准水平、控制面稳定性和治理稳健性。更重要的是，本文给出的不是某个局部模块的改进，而是一种关于**自主 IT 运维系统应如何组织**的更一般回答：与其追求一个无约束的统一大模型智能体，不如在推理、检索、执行和控制平面之间进行受治理的、可解释的、显式约束的层次化分配。本文进一步提出 Resilient Selector 反馈驱动自适应路由机制，将路由决策从 one-shot 分类升级为失败感知的渐进降级循环，通过增量上下文重丰富实现路由级、工具级和步骤级三层嵌套弹性架构，显著提升复杂运维问题的自动解决覆盖率。

未来工作应进一步关注漂移条件下的自适应重校准、回滚感知的长时程修复规划，以及跨组织、跨领域和高监管环境中的泛化评估。
