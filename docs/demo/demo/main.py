...
class MockMegaAgent:
    ...
    def __init__(
        self, name: str, model_id: str, system_prompt: str, selector_strategy: str
    ):
        self.name = name
    ...
    async def reply(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        template = self.response_templates.get(
            route_type, self.response_templates["direct"]
        )
...
class MockRAGPipeline:
    ...
    async def ingest(self, collection_id: str, documents: list) -> dict:
        return {
            ...
            "chunks_created": len(documents) * 3,
        }
...
"""
            ...
            if skill_dir.is_dir():
                ...
                if manifest_file.exists():
                    try:
                        ...
                        print(
                            f"  {colorize('✓', Colors.GREEN)} {skill.manifest.name} (v{skill.manifest.version})"
                        )
                    except Exception as e:
                        print(f"  {colorize('✗', Colors.RED)} {skill_dir.name}: {e}")

        print(
            f"  共加载 {colorize(str(len(self.loaded_skills)), Colors.CYAN, bold=True)} 个 Skills"
        )
    ...
    async def _init_rag(self) -> None:
        ...
        if documents:
            ...
            print(
                f"  共导入 {colorize(str(result['documents_processed']), Colors.CYAN, bold=True)} 个文档"
            )
        else:
            print(f"  {colorize('⚠', Colors.YELLOW)} 使用模拟文档进行演示")
            mock_docs = [
                {
                    "content": "502 Bad Gateway 错误处理...",
                    "metadata": {"topic": "502-error"},
                },
                {
                    "content": "OutOfMemoryError 处理手册...",
                    "metadata": {"topic": "oom-error"},
                },
                {
                    "content": "数据库连接池问题...",
                    "metadata": {"topic": "db-connection"},
                },
            ]
    ...
    async def _load_workflows(self) -> None:
        ...
        print(
            f"  共加载 {colorize(str(len(self.loaded_workflows)), Colors.CYAN, bold=True)} 个 Workflows"
        )
    ...
    async def _create_agent(self) -> None:
        ...
        if RESOLVEAGENT_AVAILABLE:
            self.agent = MegaAgent(
                ...
                selector_strategy=agent_conf.get("selector_config", {}).get(
                    "strategy", "hybrid"
                ),
            )
        else:
        ...
        print(
            f"  {colorize('✓', Colors.GREEN)} Agent 已创建: {colorize(self.agent.name, Colors.CYAN, bold=True)}"
        )
        print(f"    - 路由策略: {self.agent.selector_strategy}")
        print(
            f"    - 可用 Skills: {', '.join(self.loaded_skills) if self.loaded_skills else 'N/A'}"
        )
    ...
    def _print_route_decision(self, metadata: dict) -> None:
        ...
        print(
            "│"
            + colorize(
                "  🧠 INTELLIGENT SELECTOR 路由决策", Colors.CYAN, bold=True
            ).ljust(67)
            + "│"
        )
        print("├" + "─" * 58 + "┤")
        ...
        if route_target:
            target_line = (
                f"│  路由目标: {colorize(route_target, Colors.WHITE, bold=True)}"
            )
    ...
    async def run_batch_demo(self) -> None:
        ...
        scenarios = [
            {
                "name": "🔧 [SKILL·日志分析] P1 工单 — OOM 紧急分析",
                "input": "【P1 工单 #INC-20240312-017】订单服务 order-service 凌晨 02:14 开始集中报错，"
                "日志出现大量 java.lang.OutOfMemoryError: Java heap space，"
                "请分析 /data/logs/order-service/ 下的错误日志，定位可能的内存泄漏点",
                "expected_route": "skill",
                "expected_target": "log-analyzer",
                "description": "真实 P1 工单场景：服务 OOM 后需紧急分析错误日志，预期路由到 log-analyzer",
            },
            {
                "name": "🔧 [SKILL·指标检查] 告警响应 — CPU 飙升",
                "input": "【告警响应】前端网关节点 prod-gateway-01 的 cpu_usage 连续 3 个采集周期超过 95%，"
                "请检查过去 30 分钟的指标趋势，判断是流量突增还是异常进程导致",
                "expected_route": "skill",
                "expected_target": "metrics-checker",
                "description": "告警工单场景：CPU 持续高负载需检查指标，预期路由到 metrics-checker",
            },
            {
                "name": "🔧 [SKILL·Web 搜索] 升级异常 — 社区方案检索",
                "input": "【故障排查】Kubernetes 集群从 1.26 升级到 1.28 后，Pod 频繁出现 "
                "UnexpectedAdmissionError，帮我在网上搜索一下社区是否有人遇到过类似问题以及解决方案",
                "expected_route": "skill",
                "expected_target": "web-search",
                "description": "版本升级引发的未知问题需搜索社区经验，预期路由到 web-search",
            },
            {
                "name": "📚 [RAG·知识检索] 502 紧急处理流程",
                "input": "【SOP 咨询】生产环境 Nginx 返回大量 502 Bad Gateway，已影响下单链路，"
                "502 错误怎么处理？紧急处理流程中各步骤的 SLA 是什么？",
                "expected_route": "rag",
                "expected_target": "support-knowledge-base",
                "description": "故障期间紧急查阅 502 Runbook 处理流程，预期路由到 RAG 知识库",
            },
            {
                "name": "📚 [RAG·知识检索] 数据库连接池调优",
                "input": "【知识查询】RDS MySQL 连接池频繁出现 Cannot acquire connection from pool，"
                "如何调整 HikariCP 配置来优化连接池？推荐的最大连接数和超时参数是什么？",
                "expected_route": "rag",
                "expected_target": "support-knowledge-base",
                "description": "SRE 日常调优需求：查阅数据库连接池最佳实践，预期路由到 RAG 知识库",
            },
            {
                "name": "🌳 [FTA·故障诊断] P0 支付链路延迟飙升",
                "input": "【P0 故障 #INC-20240312-023】支付核心链路响应延迟从 50ms 飙升到 3s，"
                "成功率下降到 92%，已影响超过 1000 笔交易，"
                "请结合日志、指标和知识库进行系统化根因诊断",
                "expected_route": "fta",
                "expected_target": "incident-diagnosis",
                "description": "核心业务 P0 故障：需多维度协同诊断根因，预期路由到 FTA 故障树工作流",
            },
            {
                "name": "🌳 [FTA·故障诊断] 间歇性服务不可用",
                "input": "【故障工单 #INC-20240315-008】用户反馈间歇性无法访问会员中心，"
                "涉及服务 member-service 和 API 网关，现象不定时出现，"
                "需要系统性排查网络、服务状态、资源等维度，帮我诊断故障根因",
                "expected_route": "fta",
                "expected_target": "incident-diagnosis",
                "description": "间歇性故障需结构化排查：通过 FTA 多分支分析定位根因",
            },
            {
                "name": "💻 [CODE_ANALYSIS·代码审查] 支付模块死锁风险",
                "input": "【代码审查】支付模块近期上线了新的分布式锁实现，"
                "review 时怀疑有潜在死锁风险，"
                "请分析 com/payment/lock/DistributedLockManager.java 中锁获取顺序是否存在问题",
                "expected_route": "code_analysis",
                "expected_target": "static-analyzer",
                "description": "上线后稳定性隐患排查：对可疑代码进行静态分析，预期路由到代码分析引擎",
            },
            {
                "name": "💬 [DIRECT·直接对话] 容器化迁移咨询",
                "input": "我们的项目计划从虚拟机迁移到 Kubernetes 容器部署，"
                "想了解一下 ResolveAgent 平台是否支持 K8s 环境运行，部署有哪些前提条件？",
                "expected_route": "direct",
                "expected_target": "",
                "description": "架构规划咨询类问题：无明确技能/知识/诊断意图，预期由 LLM 直接回复",
            },
            {
                "name": "🔧 [SKILL·日志分析] 上线回滚日志复盘",
                "input": "【复盘工单】昨天 22:00 商品详情页 v2.3.1 上线后错误率从 0.1% 升到 4.7%，"
                "已回滚。请分析 /var/log/nginx/error.log 中该时段的异常请求日志，"
                "找出触发 500 的 Top-5 接口路径和报错模式",
                "expected_route": "skill",
                "expected_target": "log-analyzer",
                "description": "上线回滚后复盘：通过日志分析定位问题接口，预期路由到 log-analyzer",
            },
        ]
        ...
        for i, scenario in enumerate(scenarios, 1):
            print(f"\n{'─' * 60}")
            print(
                f"{colorize(f'场景 {i}:', Colors.BLUE, bold=True)} {scenario['name']}"
            )
            print(f"{'─' * 60}")
            print(f"{colorize('描述:', Colors.DIM)} {scenario['description']}")
            print(f"{colorize('输入:', Colors.WHITE, bold=True)} {scenario['input']}")
            print(
                f"{colorize('预期:', Colors.DIM)} {scenario['expected_route']}:{scenario['expected_target']}"
            )
            ...
            target_match = (
                (not expected_target)
                or (expected_target in route_target)
                or (route_target in expected_target)
            )
        ...
        print(
            colorize(
                f"测试完成: {passed}/{total} 通过 ({rate:.0f}%)",
                summary_color,
                bold=True,
            )
        )
    ...
    async def run_interactive(self) -> None:
        ...
        # 显示示例提示
        print(colorize("示例问题（工单风格）:", Colors.CYAN))
        examples = [
            "【P1】order-service 日志出现大量 OutOfMemoryError，请分析错误日志",
            "502 Bad Gateway 怎么处理？紧急流程是什么？",
            "【告警】prod-gateway-01 CPU 连续 3 个周期超 95%，请检查指标趋势",
            "支付链路延迟飙升到 3s，请结合日志和指标诊断根因",
            "帮我搜索 Kubernetes 1.28 Pod UnexpectedAdmissionError 的解决方案",
            "【代码审查】分析 DistributedLockManager.java 是否存在死锁风险",
        ]
        for ex in examples:
...
路由类型:
  ...
  direct        直接对话 - LLM 直接回复
    ...
    parser.add_argument(
        "--skills-dir", type=str, default="skills", help="Skills 目录路径"
    )

    parser.add_argument(
        "--workflows-dir", type=str, default="workflows", help="Workflows 目录路径"
    )
