...
检索增强生成（Retrieval-Augmented Generation, RAG）管道是 ResolveNet 的知识管理核心，支持文档摄取、向量索引和语义检索。
...
```bash
# 使用配置文件创建
resolvenet rag collection create -f collection-config.yaml
```
...
```python
from resolvenet.rag import RAGPipeline
...
```
原文:
"ResolveNet 是一个 Mega Agent 平台。它支持技能、工作流和 RAG。
平台采用微服务架构。服务间通过 gRPC 通信。"

固定分块 (size=30):
├── "ResolveNet 是一个 Mega Agent 平台。它"
...
语义分块:
├── "ResolveNet 是一个 Mega Agent 平台。它支持技能、工作流和 RAG。"  [产品介绍]
...
```python
from resolvenet.rag import EmbeddingModel
...
```prometheus
# 检索延迟
resolvenet_rag_retrieval_latency_seconds{collection="product-docs"}

# 检索数量
resolvenet_rag_queries_total{collection="product-docs"}

# 嵌入延迟
resolvenet_rag_embedding_latency_seconds{model="bge-large-zh"}

# 摄取速率
resolvenet_rag_ingest_documents_total{collection="product-docs"}
```
