...
class CallChainRAGGenerator:
    ...
    def _generate_overview(self, chain: CallChainData) -> RAGDocument:
        ...
        call_type_summary = ", ".join(f"{ct}: {count}" for ct, count in sorted(call_types.items()))

        # Build file summary
        file_summary = "\n".join(
            f"- **{sf.file_name}** (`{sf.file_path}`) - {sf.description} [{sf.importance}] [{sf.component}]" for sf in chain.source_files
        )

        # Build flow steps
        flow_steps_str = "\n".join(f"{idx + 1}. {step}" for idx, step in enumerate(chain.flow_steps))
...
"""
    ...
    def _generate_source_file_docs(self, chain: CallChainData) -> list[RAGDocument]:
        ...
        for sf in chain.source_files:
            ...
            incoming_str = "\n".join(f"- ← {e.label} (from {e.source_file_id}, type: {e.call_type})" for e in incoming) or "无入边"
            outgoing_str = "\n".join(f"- → {e.label} (to {e.target_file_id}, type: {e.call_type})" for e in outgoing) or "无出边"
    ...
    def _generate_function_docs(self, chain: CallChainData) -> list[RAGDocument]:
        ...
        for sf in chain.source_files:
            for fn in sf.key_functions:
                ...
                related_edges = [e for e in chain.edges if fn.name in e.functions]

                edge_context = (
                    "\n".join(f"- {e.label} ({e.source_file_id} → {e.target_file_id}, type: {e.call_type})" for e in related_edges) or "无直接关联边"
                )
...
"""
    ...
    def _generate_flow_docs(self, chain: CallChainData) -> list[RAGDocument]:
        ...
        all_steps = "\n".join(f"**步骤 {idx + 1}**: {step}" for idx, step in enumerate(chain.flow_steps))
...
"""
        ...
        for comp, files in component_map.items():
            comp_edges_out = [e for e in chain.edges if any(f.id == e.source_file_id for f in files)]
            comp_edges_in = [e for e in chain.edges if any(f.id == e.target_file_id for f in files)]

            all_functions = [fn for sf in files for fn in sf.key_functions]
    ...
    def _generate_qa_pairs_docs(self, chain: CallChainData) -> list[RAGDocument]:
        ...
        qa_pairs.append(
            (
                ...
                "涉及以下源码文件：\n" + "\n".join(f"- {sf.file_name} ({sf.file_path}): {sf.description}" for sf in chain.source_files),
            )
        )
        ...
        if chain.chain_type == "troubleshooting":
            ...
            critical_fns = [(sf, fn) for sf in chain.source_files if sf.importance == "critical" for fn in sf.key_functions]
            if critical_fns:
                qa_pairs.append(
                    (
                        f"{chain.name} 中最关键的函数有哪些？",
                        "最关键的函数包括：\n" + "\n".join(f"- {fn.name} ({sf.file_name}): {fn.description}" for sf, fn in critical_fns),
                    )
                )
            ...
            if async_edges:
                qa_pairs.append(
                    (
                        ...
                        "异步/事件驱动调用包括：\n"
                        + "\n".join(f"- {e.label} ({e.source_file_id} → {e.target_file_id}, 类型: {e.call_type})" for e in async_edges),
                    )
                )
            ...
            qa_pairs.append(
                (
                    ...
                    "初始化阶段按以下顺序执行：\n" + "\n".join(f"阶段 {idx + 1}: {step}" for idx, step in enumerate(chain.flow_steps)),
                )
            )
...
"""
    ...
    @staticmethod
    def _format_edge_list(edges: list[ChainEdge], *, direction: str = "out") -> str:
        """Format a list of edges for Markdown output."""
        lines: list[str] = []
        for e in edges:
            if direction == "out":
                lines.append(f"- \u2192 {e.label} (to {e.target_file_id}, type: {e.call_type})")
            else:
                lines.append(f"- \u2190 {e.label} (from {e.source_file_id}, type: {e.call_type})")
        return "\n".join(lines)
