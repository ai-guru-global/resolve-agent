...
- [ ] **Step 1: 确认 .gitignore 规则已覆盖三类产物（只验证，不改动）**
...
- [ ] **Step 2: 从版本控制移除 .pids/*.pid（保留磁盘文件）**
...
- [ ] **Step 1: 搬移 9 个内容文件到 docs/（目标目录先建）**
...
- [ ] **Step 2: 在原路径写 9 个 stub 页（Docusaurus id 不变，仅引导到唯一源）**
...
- [ ] **Step 3: 重写 docs-site/sidebars.ts（原配置引用 19 个 id，其中 9 个从未存在导致站点不可构建；改为仅 10 个真实 id）**
...
- [ ] **Step 4: 修正 docs-site/docusaurus.config.ts（五处）**
...
- [ ] **Step 6: 修正 docs/design 三篇的 ADR 链接前缀**
...
- [ ] **Step 9: （可选 best-effort）Docusaurus 构建验证**
...
- [ ] **Step 2: 修正 ci.yaml test-mobile job**
...
- [ ] **Step 4: 修正 docs/design/15-web-frontend.md**
